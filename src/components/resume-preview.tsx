      ...
      <div className='bg-white text-slate-800 w-full max-w-[210mm] min-h-[297mm] mx-auto shadow-2xl overflow-hidden font-montserrat flex flex-col'>
        ...
        <div className='flex h-40 m-4'>
          ...
          <div className='w-[40%] bg-black text-white flex flex-col justify-center px-8 space-y-3 text-xs'>
            ...
            {location.city && (
              <ContactItem icon={MapPin} text={`${location.city}, ${location.region}`} />
            )}
        ...
        <div className='flex flex-1'>
          ...
          <div className='w-[35%] bg-slate-200 p-8 space-y-10'>
            {education.length > 0 && (
              <section>
                <h2 className='text-xl uppercase font-light text-slate-700 tracking-wider mb-2 border-b-2 border-slate-400 pb-1'>
                  Formación
                </h2>
                  ...
                  {education.map((edu: any, index: number) => (
                    <div key={index} className='text-sm'>
                      ...
                      <p className='text-slate-600 mb-2'>
                        {edu.institution} | {edu.startDate.split("-")[0]} -{" "}
                        {edu.endDate ? edu.endDate.split("-")[0] : "Presente"}
                      </p>
                ...
                <ul className='mt-6 space-y-2'>
                  {languages.map((lang: any, index: number) => (
                    <li key={index} className='text-sm'>
                      ...
                      <span className='text-slate-600'>{lang.fluency}</span>
                    </li>
                  ))}
                </ul>
              </section>
          ...
          <div className='flex-1 p-10 space-y-12'>
            {summary && (
              <section>
                <h2 className='text-xl uppercase font-medium text-slate-700 tracking-wider mb-4 border-b border-slate-300 pb-2'>
                  Sobre mí
                </h2>
            ...
            {work.length > 0 && (
              <section>
                <h2 className='text-xl uppercase font-medium text-slate-700 tracking-wider mb-6 border-b border-slate-300 pb-2'>
                  Experiencia
                </h2>
                    ...
                    <div key={index} className='pl-8 relative'>
                      ...
                      <div className='mb-1'>
                        ...
                        <p className='text-xs font-semibold text-slate-500 uppercase mt-1'>
                          {job.position} <span className='mx-1'>|</span> {job.startDate} -{" "}
                          ...
                          {project.url && (
                            <>
                              ...
                              <a
                                ...
                                target='_blank'
                                rel='noreferrer'
                                className='lowercase hover:text-blue-600 transition-colors'
                              >
      ...
      <div className='bg-white text-slate-800 w-full max-w-[210mm] min-h-[297mm] mx-auto shadow-2xl overflow-hidden font-sans flex flex-col'>
        ...
        <div className='flex flex-1 p-10 gap-10'>
          ...
          <div className='flex-1 space-y-8'>
            {summary && (
              <section>
                <h2 className='text-lg font-bold text-gray-800 uppercase tracking-wider border-b border-gray-300 pb-2 mb-3'>
                  Perfil Profesional
                </h2>
            ...
            {work.length > 0 && (
              <section>
                <h2 className='text-lg font-bold text-gray-800 uppercase tracking-wider border-b border-gray-300 pb-2 mb-4'>
                  Experiencia Laboral
                </h2>
    ...
    return (
      <div className='bg-white text-slate-900 w-full max-w-[210mm] min-h-[297mm] mx-auto shadow-2xl overflow-hidden font-serif p-16 flex flex-col'>
        ...
        <div className='space-y-8'>
          {work.length > 0 && (
            <section>
              <h2 className='text-lg font-bold uppercase tracking-widest border-b border-slate-300 pb-1 mb-4'>
          ...
          <div className='mt-12 space-y-6 text-sm'>
            <section>
              ...
              <div className='space-y-3 text-slate-300'>
                ...
                {location.city && (
                  <div className='flex items-center gap-3'>
                    <MapPin className='w-4 h-4' /> <span>{location.city}</span>
                  </div>
                )}
              </div>
            </section>
                  ...
                  {education.map((edu: any, index: number) => (
                    <div key={index} className='bg-slate-50 p-4 rounded-lg border border-slate-100'>
                      ...
                      <p className='text-sm text-slate-600 mb-1'>
                        {edu.studyType} en <span className='font-semibold'>{edu.area}</span>
                      </p>
        ...
        <div className='space-y-8'>
          ...
          {summary && (
            <section>
              ...
              <p className='text-slate-600 leading-relaxed text-justify border-l-4 border-slate-100 pl-4'>
                {summary}
              </p>
            </section>
          )}
                ...
                {work.map((job: any, index: number) => (
                  <div key={index}>
                    ...
                    <ul className='list-disc list-inside space-y-1 text-sm text-slate-600 leading-relaxed'>
                      {job.summary && (
                        <li className='marker:text-[#0f172a] text-justify'>{job.summary}</li>
                      )}
                      {job.highlights &&
                        job.highlights.map((highlight: string, i: number) => (
                          <li key={i} className='marker:text-[#0f172a] text-justify'>
                            {highlight}
                          </li>
                        ))}
                    </ul>
          ...
          {(skills.length > 0 || languages.length > 0) && (
            <section>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 bg-slate-50 p-6 rounded-lg border border-slate-100 gap-8'>
                <div>
                  <h3 className='font-bold text-[#0f172a] border-b-2 border-slate-200 pb-2 mb-3'>
                      ...
                      <div key={index}>
                        ...
                        <div className='flex flex-wrap gap-1.5'>
                          {skill.keywords?.map((keyword: string, kIndex: number) => (
                            <span
                              key={kIndex}
                              className='px-2 py-0.5 bg-white border border-slate-200 rounded text-[10px] font-medium text-slate-600'
                            >
                              {keyword}
                            </span>
                          ))}
                  ...
                  <div>
                    ...
                    <ul className='space-y-2'>
                      {languages.map((lang: any, index: number) => (
                        <li
                          key={index}
                          className='text-sm flex justify-between items-center bg-white p-2 rounded border border-slate-100'
                        >
                          <span className='font-medium text-slate-700'>{lang.language}</span>
                          <span className='text-xs text-white bg-[#0f172a] px-2 py-0.5 rounded'>
                            {lang.fluency}
                          </span>
                        </li>
                      ))}
